...
export default function Home() {
  // Estado para el idioma (puedes obtenerlo de localStorage o de un contexto)
  const [language, setLanguage] = useState('es'); // Valor predeterminado: español

  // Opcional: Cargar la configuración del idioma al inicio
  useEffect(() => {
    // Ejemplo: obtener el idioma guardado en localStorage
      ...
      <div className="relative z-10">
        ...
        <Link href="/shop">
          <button className="flex-1 btn-animated py-3 px-6 glass-effect border border-accent/20 text-accent rounded-full transition-all duration-300 hover:bg-accent/10">
            ...
            <span>{language === 'en' ? 'Visit Store' : 'Visitar Tienda'}</span>
          </button>
        </Link>
      </div>
    </div>
